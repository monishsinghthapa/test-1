...
import { ActionType, AppContext } from "@/app/Appcontext/AppContext";
import { setPageNumber } from "@/app/Appcontext/Action";
...
export function ChatSources({ data }: { data: SourceData }) {
  ...
  const onClickSource = (node: SourceNode) => {

    setPageNumber(dispatch, node.metadata)
  }
      ...
      <div className="flex flex-wrap gap-1 items-center mt-4">
        {sources.map((node: SourceNode, index: number) => (
          <div key={node.id}>
            <HoverCard>
              <HoverCardTrigger>
                <div onClick={() => onClickSource(node)} className="text-xs px-4 py-2 rounded-full bg-gray-300 mb-2  flex items-center justify-center hover:text-white hover:bg-primary hover:cursor-pointer">
                  <span>{node.metadata.file_name} (Page No. {node.metadata.page_label}) </span>
                </div>
              </HoverCardTrigger>
              {/* <HoverCardContent>
                <NodeInfo node={node} />
